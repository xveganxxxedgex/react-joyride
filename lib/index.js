...
function getScrollTo(element, offset) {
  ...
  var scrollKey = axis === 'y' ? 'scrollTop' : 'scrollLeft';
  var offsetKey = axis === 'y' ? 'top' : 'left';
  var parentPos = parent.getBoundingClientRect()[offsetKey];
  var elementOffset = element.getBoundingClientRect()[offsetKey];
  var scrollToPos = parent[scrollKey] + elementOffset;
...
function getElementPosition(element, offset) {
  ...
  var isYAxis = axis === 'y';
  var rectKey = isYAxis ? 'top' : 'left';
  var scrollKey = isYAxis ? 'scrollTop' : 'scrollLeft';

  var position = elementRect[rectKey] + (!hasScrollParent && !isFixed(element) ? scrollParent[scrollKey] : 0);
...
function scrollTo(value) {
  ...
  return new Promise(function (resolve, reject) {
    var isYAxis = axis === 'y';
    var scrollMethod = isYAxis ? 'top' : 'left';
    var scrollKey = isYAxis ? 'scrollTop' : 'scrollLeft';
    var scrollValue = element[scrollKey];

    var limit = value > scrollValue ? value - scrollValue : scrollValue - value;

    scroll[scrollMethod](element, value, { duration: limit < 100 ? 50 : 300 }, function (error) {
...
var Overlay = function (_React$Component) {
  ...
  createClass(Overlay, [{
    ...
    get: function get$$1() {
      ...
      var left = getElementPosition(element, spotlightPadding, 'x');
...
var Joyride = function (_React$Component) {
  ...
  createClass(Joyride, [{
    ...
    key: 'getScrollValue',
    value: function getScrollValue(step, target, axis) {
      var lifecycle = this.state.lifecycle;
      var scrollOffset = this.props.scrollOffset;

      var hasCustomScroll = hasCustomScrollParent(target);
      var scrollValue = Math.floor(getScrollTo(target, scrollOffset, axis));
      ...
      if (lifecycle === LIFECYCLE.BEACON && this.beaconPopper) {
        ...
        if (!hasCustomScroll && !([rectEnd].indexOf(placement) !== -1)) {
    ...
    value: function scrollToStep(prevState) {
      ...
      var target = getElement(step.target);
      var scrollToStep = scrollToFirstStep || prevState.index !== index;
      var fixedStep = step.isFixed || isFixed(target);
      var lifecycleReady = prevState.lifecycle !== lifecycle && [LIFECYCLE.BEACON, LIFECYCLE.TOOLTIP].indexOf(lifecycle) !== -1;

      var shouldScroll = !disableScrolling && !fixedStep // fixed steps don't need to scroll
      && lifecycleReady && scrollToStep;

      if (status === STATUS.RUNNING && shouldScroll) {
        var scrollParent = getScrollParent(target);
        var scrollY = this.getScrollValue(step, target, 'y');
        var scrollX = this.getScrollValue(step, target, 'x');
